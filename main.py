...
def load_model():
    global model
    if model is None:
        model = pickle.load(open('F:\\MegaProject\\model\\RFC.pkl', 'rb'))  # Use double backslashes for Windows paths
    return model

def load_treatment_data():
    global treatment_data
    if treatment_data is None:
        treatment_data = pd.read_csv(r'F:\MegaProject\Dataset\treatment.csv', encoding='latin1')  # Adjust encoding if needed
